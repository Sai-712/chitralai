...
const UploadSelfie = () => {
  ...
  useEffect(() => {
    const initializeComponent = async () => {
      try {
        ...
        if (!shortidRegex.test(urlEventId)) {
  ...
  const captureImage = async () => {
    ...
    try {
      ...
      setSelectedImage(URL.createObjectURL(cameraFile));
          ...
          <div className="max-w-2xl mx-auto bg-white p-8 rounded-lg shadow-md border-2 border-blue-700">
            ...
            {selectedImage && (
              <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
                ...
                className="absolute bottom-4 right-4 p-2 rounded-full bg-black/60 text-white hover:bg-black/70 transition-colors duration-200 flex items-center gap-2"
              >
